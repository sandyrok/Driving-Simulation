...
def compute(first,second):
	tenFirst = torch.FloatTensor(numpy.ascontiguousarray(first[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
	tenSecond = torch.FloatTensor(numpy.ascontiguousarray(second[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
	...
	a = numpy.array(tenOutput.numpy().transpose(1, 2, 0), numpy.float32)
	return a[:,:,0],a[:,:,1]
# end
